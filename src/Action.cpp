...
void PrintMenu::act(Restaurant &restaurant) {
    ...
    for (int i = 0; (unsigned) i < restaurant.getMenu().size(); i++) {
        switch (restaurant.getMenu()[i].getType()) {
            case VEG:
                type = "VEG";
                break;
            case SPC:
...
void MoveCustomer::act(Restaurant &restaurant) {
    ...
    if (restaurant.getTable(srcTable)->isOpen() == false || restaurant.getTable(dstTable)->isOpen() == false)
        ...
        this->error("Cannot move customer");
    else if (restaurant.getTable(dstTable)->getCustomers().size() >=
             (unsigned) restaurant.getTable(dstTable)->getCapacity())
        this->error("Cannot move customer");
        ...
        for (int i = 0; (unsigned) i < orders.size(); i++) {
            if (orders[i].first == id) {//Checking which orders the client that is about to move,ordered.
                restaurant.getTable(dstTable)->getOrders().push_back(
                        restaurant.getTable(srcTable)->getOrders()[i]);//Adding order to the destination table.
                restaurant.getTable(dstTable)->setBill(restaurant.getTable(dstTable)->getBill() +
                                                       orders[i].second.getPrice());//Updating destination table's bill.
                restaurant.getTable(srcTable)->setBill(restaurant.getTable(srcTable)->getBill() -
                                                       orders[i].second.getPrice());//Updating source table's bill.

            }
        }
        std::vector<OrderPair> copy(
                restaurant.getTable(srcTable)->getOrders());//Making a copy of the source table's orders.
        ...
        restaurant.getTable(dstTable)->getCustomers().push_back(
                restaurant.getTable(srcTable)->getCustomer(id));//Pushing new customer to destination table.
